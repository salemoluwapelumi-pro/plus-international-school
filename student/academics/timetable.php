...
$term = current_term();
$slots = $user['class_id'] ? TimetableManager::forClass((int) $user['class_id'], $sessionName, $term) : [];
$grid = [];
...
<div class="panel">
                ...
                <tr>
                    ...
                    <?php foreach (TimetableManager::DAYS as $day): $slot = $grid[$day][$period] ?? null; ?>
                        <td class="slot <?= $slot ? 'filled' : '' ?>">
                            <?php if ($slot): ?>
                                <strong><?= e($slot['subject_name']) ?></strong><small><?= e($slot['teacher_name'] ?? 'Teacher to be assigned') ?></small>
    ...
    <?php if (!$slots): ?><div class="empty-state">Your class timetable has not been published yet.</div><?php endif; ?>
</div>
